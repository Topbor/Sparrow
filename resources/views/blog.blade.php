         ...
         @forelse ($article as $atc)
            <article class="post">
               ...
               <div class="entry-header cf">
                  ...
                  <h1><a href="{{route('blog.view' ,['id' => $atc->id])}}" title="">{{$atc->title}}</a></h1>
                  <p class="post-meta" style="margin:10px;">
                     <time class="date">Додано: {{$atc->added_on}}</time>
                  </p>
                  <p>{{$atc->description}}<a href="{{route('blog.view' ,['id' => $atc->id])}}"> Дізнайтесь більше тут</a></p>
                  
               </div>

               <div class="post-thumb">
                  <a href="{{route('blog.view' ,['id' => $atc->id])}}" title=""><img src="{{$atc->image_url}}" alt="post-image" title="post-image"></a>
               </div>
